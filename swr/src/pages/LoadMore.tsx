...
const LoadMorePage = () => {
    ...
    const { data: posts, size, setSize, isLoading, error, isValidating } = useSWRInfinite(getKey, fetcher, { initialSize: 1 })
    ...
    return (
        <>
            ...
            {isValidating && <div style={{ inset: 0 }} className="spinner-border d-block m-auto my-3" role="status"></div>}
            <hr />

            <div className='d-flex justify-content-center gap-1 align-items-center'>
                <button disabled={size === toatalPages} className='btn btn-sm btn-primary' onClick={() => setSize(size + 1)}>Load More</button>
            </div>
        </>
    )
}
